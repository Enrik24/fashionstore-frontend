...
@Component({
    ...
    <div class="page-container animate-fade-in">
        ...
        <div class="page-header-info">
          ...
          <div>
            ...
            <p class="page-subtitle">
              Consulta de ventas online y presenciales de
              <strong>{{ authService.userSucursalNombre() || 'tu sucursal' }}</strong>.
      ...
      @if (!sucursalId()) {
        <div class="card warning-box">
          <i class="ri-alert-line"></i> Tu usuario no tiene una sucursal asignada. Contacta al administrador.
        </div>
      } @else {
        <div class="attr-tabs-container card">
          <div class="attr-tabs">
            <button class="attr-tab" [class.active]="canal() === 'online'" (click)="setCanal('online')">
              ...
              <span class="tab-count">{{ ordenesSucursal().length }}</span>
            </button>
        ...
        @if (isLoading()) {
          <div class="card loading-box"><i class="ri-loader-4-line spin"></i> Cargando ventas de la sucursal...</div>
        } @else if (canal() === 'online') {
          <div class="card table-card">
            <table class="data-table">
              <thead><tr><th>N° Orden</th><th>Fecha</th><th>Estado</th><th class="num">Total</th><th></th></tr></thead>
              <tbody>
                @for (o of paginaOnline(); track o.id) {
                  ...
                  <tr><td colspan="5" class="empty-cell">No hay ventas online realizadas en tu sucursal.</td></tr>
                }
              </tbody>
            </table>
            <div class="pager">
          ...
          <div class="card table-card">
            <table class="data-table">
              <thead><tr><th># Venta</th><th>Fecha</th><th>Método pago</th><th class="num">Total</th><th></th></tr></thead>
              <tbody>
                @for (v of paginaPresencial(); track v.id) {
                  ...
                  <tr><td colspan="5" class="empty-cell">No hay ventas presenciales en tu sucursal.</td></tr>
                }
              </tbody>
            </table>
            <div class="pager">
  ...
  styles: [`
    ...
    .warning-box { padding: 1.25rem; display: flex; align-items: center; gap: 0.6rem; color: #b45309; background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.25); }
...
export class BranchSalesComponent implements OnInit {
  ...
  sucursalId = computed(() => this.authService.userSucursalId());

  /** Órdenes DIGITAL de la sucursal del encargado (el backend no filtra por sucursal: se filtra aquí). */
  ordenesSucursal = computed(() =>
    this.ordenes().filter(o => o.sucursal_id === this.sucursalId() && (o.tipo as string) !== 'PRESENCIAL')
  );

  filtradasOnline = computed(() => {
    const q = this.busqueda.trim().toLowerCase();
    return this.ordenesSucursal().filter(o => {
  ...
  ngOnInit(): void {
    ...
    this.posService.getInPersonSales(0, 100, sid ?? undefined).subscribe({
  ...
  verDetallePresencial(v: VentaPresencial): void {
    ...
    if (!v.orden && (v as any).orden_id) {
      this.orderService.getOrderById((v as any).orden_id).subscribe({
