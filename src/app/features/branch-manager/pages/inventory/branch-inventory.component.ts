...
@Component({
    ...
    <div class="page-container animate-fade-in">
      ...
      <div class="page-header-card card">
        <div class="page-header-left">
          <div class="page-icon-box"><i class="ri-archive-line"></i></div>
          <div>
            <h1 class="page-title">Inventario de Sucursal</h1>
            <p class="page-subtitle">
              <i class="ri-map-pin-2-fill text-accent"></i>
              {{ authService.userSucursalNombre() || 'Sucursal asignada' }}
              — Control de existencias y movimientos de prendas
            </p>
          </div>
        </div>
      ...
      <div class="card inventory-main-card">
        <div class="table-card-header">
          <div class="header-count">
            <h3 class="table-title">Existencias en Tu Sucursal</h3>
          ...
          <div class="header-actions">
            <div class="expand-collapse-group">
              <button type="button" class="btn-text-action" (click)="expandAll()">
                <i class="ri-expand-vertical-line"></i> Expandir todo
              </button>
              <span class="action-divider">|</span>
              <button type="button" class="btn-text-action" (click)="collapseAll()">
        ...
        @if (isLoading()) {
          <div class="loading-state py-12 text-center">
            ...
            <p class="text-muted mt-3">Cargando existencias de tu sucursal...</p>
          </div>
          ...
          <div class="empty-state py-12 text-center">
            ...
            <p class="text-muted">Prueba cambiando los filtros de búsqueda o estado.</p>
          </div>
        } @else {
              ...
              <div class="product-accordion-item" [class.is-expanded]="isProductExpanded(group.productId)">
                <div class="accordion-header" (click)="toggleProduct(group.productId)">
                  <div class="accordion-header-left">
                    <button type="button" class="toggle-chevron-btn" [class.expanded]="isProductExpanded(group.productId)">
                  ...
                  <div class="accordion-header-right">
                    <div class="metric-pill variants-count-pill">
                      ...
                      <span><strong>{{ group.variantsCount }}</strong> {{ group.variantsCount === 1 ? 'variante' : 'variantes' }}</span>
                    </div>
                            ...
                            <tr [class.row-alert]="item.cantidad <= item.stock_minimo">
                              ...
                              <td>
                                <div class="color-badge-box">
                                  @if (item.variante_producto?.color?.codigo_hex) {
                                    <span class="color-dot" [style.background]="item.variante_producto?.color?.codigo_hex"></span>
                                  }
                                  <span class="color-name">{{ item.variante_producto?.color?.nombre || '-' }}</span>
                                </div>
                              </td>
                              <td>
                                <div class="stock-cell">
                                  <span class="stock-number" [class.text-danger]="item.cantidad <= item.stock_minimo">{{ item.cantidad }}</span>
                                  <span class="min-stock-hint">(mín. {{ item.stock_minimo }})</span>
                                </div>
                              </td>
                              <td><span class="text-xs text-muted">{{ item.cantidad_reservada }} u.</span></td>
                              <td><span class="text-xs text-muted">{{ item.cantidad_vendida }} u.</span></td>
                              <td>
                              ...
                              <td>
                                <div class="action-buttons-flex">
                                  <button type="button" class="btn-action-icon" title="Ajuste Rápido" (click)="openQuickAdjustModal(item)">
                                    <i class="ri-equalizer-line"></i>
                                  </button>
                                  <button type="button" class="btn-action-icon btn-action-accent" title="Registrar Movimiento" (click)="openRegisterMovementModal(item)">
          ...
          <app-pagination
            ...
            [totalItems]="groupedInventory().length"
            [pageSize]="pageSize()"
      ...
      <app-modal
        ...
        subtitle="Registra entradas, salidas o ajustes de stock en tu sucursal"
        icon="ri-arrow-up-down-line"
        (closeEvent)="closeMovementModal()"
      >
        <form [formGroup]="movementForm" (ngSubmit)="saveMovement()" class="modal-form">
          <div class="form-group">
            <label class="form-label" for="m-inv">Registro de Inventario <span class="required">*</span></label>
            <select id="m-inv" formControlName="inventario_id" class="form-control" (change)="onMovementInventoryChange($event)">
              @for (inv of inventoryList(); track inv.id) {
                <option [value]="inv.id">
                  {{ inv.variante_producto?.producto?.nombre || 'Prenda #' + inv.variante_producto_id }} [{{ getVariantLabel(inv) }}] (Stock: {{ inv.cantidad }} u.)
                </option>
              }
            </select>
          </div>
            ...
            <div class="selected-variant-card">
              <div class="variant-preview-header">
                ...
                <div class="preview-info">
                  <span class="preview-title">{{ inv.variante_producto?.producto?.nombre || 'Prenda' }}</span>
                  <span class="preview-sku">SKU: {{ inv.variante_producto?.producto?.sku || 'N/A' }}</span>
                </div>
              </div>
              <div class="preview-badges">
                <span class="preview-badge"><i class="ri-ruler-2-line"></i> Talla: <strong>{{ inv.variante_producto?.talla?.valor || 'Sin Talla' }}</strong></span>
                <span class="preview-badge"><i class="ri-palette-line"></i> Color: <strong>{{ inv.variante_producto?.color?.nombre || 'Sin Color' }}</strong></span>
                <span class="preview-badge stock-badge"><i class="ri-inbox-archive-line"></i> Stock: <strong>{{ inv.cantidad }} u.</strong></span>
              </div>
            </div>
          }
          ...
          <div class="grid grid-cols-2 form-row">
            ...
            <div class="form-group">
              <label class="form-label" for="m-cant">Cantidad <span class="required">*</span></label>
              <input id="m-cant" type="number" min="1" formControlName="cantidad" class="form-control" placeholder="10">
            </div>
          </div>

          <div class="form-group">
            <label class="form-label" for="m-motivo">Motivo <span class="required">*</span></label>
            <textarea id="m-motivo" rows="3" formControlName="motivo" class="form-control" placeholder="Ej. Recepción de lote de proveedor..."></textarea>
          </div>
      ...
      <app-modal
        ...
        subtitle="Modifica la cantidad física total registrada"
            ...
            <div class="selected-variant-card">
              <div class="variant-preview-header">
                ...
                <div class="preview-info">
                  ...
                  <span class="preview-sku">SKU: {{ item.variante_producto?.producto?.sku || 'N/A' }}</span>
                </div>
              </div>
              <div class="preview-badges mt-2">
                <span class="preview-badge"><i class="ri-ruler-2-line"></i> Talla: <strong>{{ item.variante_producto?.talla?.valor || 'Sin Talla' }}</strong></span>
      ...
      <app-modal
        ...
        subtitle="Registro de transacciones en tu sucursal"
                ...
                @if (movementsHistory().length === 0) {
                  ...
                  @for (m of movementsHistory(); track m.id) {
                    <tr>
                        ...
                        @if (m.inventario?.variante_producto?.producto?.nombre) {
                          <div class="text-xs">
                            ...
                            <div class="text-muted">
                              Talla: {{ m.inventario?.variante_producto?.talla?.valor || '-' }} |
                      ...
                      <td><span class="badge badge-primary">{{ m.tipo }}</span></td>
  ...
  styles: [`
    ...
    .page-header-card { display: flex; align-items: center; justify-content: space-between; padding: 1.25rem 1.5rem; }
    .page-header-left { display: flex; align-items: center; gap: 1rem; }
    .page-icon-box { width: 48px; height: 48px; border-radius: var(--radius-md); background: var(--primary); color: white; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; flex-shrink: 0; }
    .page-title { font-size: 1.35rem; font-weight: 800; color: var(--primary); margin: 0; }
    .page-subtitle { color: var(--text-muted); font-size: 0.85rem; margin: 0; display: flex; align-items: center; gap: 0.35rem; }
    ...
    .toolbar-left { display: flex; align-items: center; gap: 1rem; flex: 1; flex-wrap: wrap; }
    .search-input-box { position: relative; min-width: 280px; }
    ...
    .select-filter { min-width: 170px; }
    .toolbar-right { display: flex; gap: 0.75rem; }

    .inventory-main-card { padding: 0; overflow: hidden; border: 1px solid var(--border-color); border-radius: var(--radius-md); background: var(--bg-card); box-shadow: var(--shadow-sm); }
    .table-card-header { display: flex; align-items: center; justify-content: space-between; padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border-color); background: white; flex-wrap: wrap; gap: 1rem; }
    .header-count { display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap; }
    .table-title { font-size: 1.15rem; font-weight: 700; color: var(--primary); margin: 0; margin-right: 0.5rem; }
    .count-badge { background: #f1f5f9; color: var(--secondary); font-size: 0.75rem; font-weight: 600; padding: 0.25rem 0.65rem; border-radius: var(--radius-full); display: inline-flex; align-items: center; gap: 0.35rem; &.count-badge-primary { background: rgba(15,23,42,0.08); color: var(--primary); } }
    .header-actions { display: flex; align-items: center; gap: 1rem; flex-wrap: wrap; }
    .expand-collapse-group { display: flex; align-items: center; gap: 0.5rem; font-size: 0.8125rem; }
    .btn-text-action { background: transparent; border: none; color: var(--secondary); font-size: 0.8125rem; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.25rem 0.5rem; border-radius: var(--radius-sm); transition: all var(--transition-fast); &:hover { background: #f1f5f9; color: var(--accent); } }
    .action-divider { color: var(--border-color); }

    .accordion-inventory-list { display: flex; flex-direction: column; }
    .product-accordion-item { border-bottom: 1px solid var(--border-color); transition: background-color var(--transition-fast); &:last-child { border-bottom: none; } &.is-expanded { background: #fafcff; } }
    .accordion-header { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.5rem; cursor: pointer; user-select: none; gap: 1.5rem; transition: background-color var(--transition-fast); &:hover { background: rgba(15,23,42,0.02); } }
    .accordion-header-left { display: flex; align-items: center; gap: 1rem; flex: 1; min-width: 0; }
    .toggle-chevron-btn { width: 28px; height: 28px; border-radius: var(--radius-sm); border: 1px solid var(--border-color); background: white; color: var(--secondary); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: transform var(--transition-normal), background-color var(--transition-fast); flex-shrink: 0; font-size: 1.15rem; &.expanded { transform: rotate(90deg); background: var(--primary); color: white; border-color: var(--primary); } }
    .product-thumb-box { width: 44px; height: 44px; border-radius: var(--radius-sm); overflow: hidden; flex-shrink: 0; background: #f1f5f9; border: 1px solid var(--border-color); display: flex; align-items: center; justify-content: center; }
    .product-thumb-img { width: 100%; height: 100%; object-fit: cover; }
    .product-thumb-fallback { font-size: 1.35rem; color: var(--text-muted); }
    .product-main-info { display: flex; flex-direction: column; gap: 0.25rem; min-width: 0; }
    .product-name-row { display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap; }
    .product-title { font-size: 1rem; font-weight: 700; color: var(--primary); margin: 0; }
    .category-pill { font-size: 0.7rem; font-weight: 600; padding: 0.15rem 0.5rem; border-radius: var(--radius-full); background: rgba(15,23,42,0.06); color: var(--secondary); }
    .product-sub-row { display: flex; align-items: center; gap: 0.85rem; font-size: 0.775rem; color: var(--text-muted); }
    .sku-tag { font-family: monospace; font-weight: 600; color: var(--secondary); background: #f1f5f9; padding: 0.1rem 0.4rem; border-radius: var(--radius-sm); }
    .accordion-header-right { display: flex; align-items: center; gap: 1rem; flex-shrink: 0; }
    .metric-pill { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.35rem 0.75rem; border-radius: var(--radius-md); font-size: 0.8125rem; background: white; border: 1px solid var(--border-color); &.variants-count-pill { color: var(--secondary); background: #f8fafc; i { color: var(--accent); } } &.stock-total-pill { background: #f0fdf4; border-color: #bbf7d0; color: #166534; .pill-label { font-size: 0.75rem; color: #15803d; } .pill-value { font-size: 0.9375rem; } &.stock-warn { background: #fffbeb; border-color: #fde68a; color: #92400e; .pill-label { color: #b45309; } } &.stock-zero { background: #fef2f2; border-color: #fecaca; color: #991b1b; .pill-label { color: #b91c1c; } } } }

    .accordion-body { background: #ffffff; padding: 0 1.5rem 1.25rem 3.5rem; border-top: 1px dashed var(--border-color); }
    .variant-table-wrapper { background: #f8fafc; border: 1px solid var(--border-color); border-radius: var(--radius-md); overflow: hidden; margin-top: 0.75rem; }
    .variant-subtable { margin-bottom: 0; thead th { background: #f1f5f9; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.03em; color: var(--secondary); padding: 0.65rem 1rem; border-bottom: 1px solid var(--border-color); } tbody td { padding: 0.65rem 1rem; font-size: 0.85rem; vertical-align: middle; border-bottom: 1px solid #edf2f7; } tbody tr:last-child td { border-bottom: none; } tbody tr:hover { background: #f1f5f9; } }
    .size-badge { display: inline-flex; align-items: center; justify-content: center; min-width: 32px; padding: 0.2rem 0.5rem; background: var(--primary); color: white; font-weight: 700; font-size: 0.8125rem; border-radius: var(--radius-sm); }
    .color-badge-box { display: inline-flex; align-items: center; gap: 0.45rem; }
    .color-dot { width: 14px; height: 14px; border-radius: 50%; border: 1px solid rgba(0,0,0,0.15); flex-shrink: 0; }
    .color-name { font-weight: 500; color: var(--secondary); }
    .stock-cell { display: flex; align-items: baseline; gap: 0.35rem; }
    .stock-number { font-weight: 700; font-size: 0.9375rem; color: var(--primary); &.text-danger { color: var(--error); } }
    .min-stock-hint { font-size: 0.7rem; color: var(--text-muted); }
    .available-qty { font-weight: 600; color: #047857; &.text-danger { color: var(--error); } }
    .btn-action-icon { width: 30px; height: 30px; border-radius: var(--radius-sm); border: 1px solid var(--border-color); background: white; color: var(--secondary); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; transition: all var(--transition-fast); &:hover { background: #f8fafc; color: var(--primary); border-color: var(--primary); } &.btn-action-accent:hover { background: var(--accent); color: white; border-color: var(--accent); } }
    .action-buttons-flex { display: flex; align-items: center; justify-content: flex-end; gap: 0.4rem; }
    .row-alert { background: rgba(239,68,68,0.03); }

    .selected-variant-card { background: #f8fafc; border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 0.875rem 1rem; display: flex; flex-direction: column; gap: 0.6rem; }
    .variant-preview-header { display: flex; align-items: center; gap: 0.75rem; }
    .preview-icon { width: 36px; height: 36px; border-radius: var(--radius-sm); background: rgba(15,23,42,0.08); color: var(--primary); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; flex-shrink: 0; }
    .preview-info { display: flex; flex-direction: column; gap: 0.15rem; }
    .preview-title { font-weight: 600; font-size: 0.95rem; color: var(--primary); }
    .preview-sku { font-size: 0.75rem; color: var(--text-muted); }
    .preview-badges { display: flex; flex-wrap: wrap; gap: 0.5rem; }
    .preview-badge { display: inline-flex; align-items: center; gap: 0.35rem; font-size: 0.75rem; background: white; border: 1px solid var(--border-color); border-radius: var(--radius-sm); padding: 0.25rem 0.6rem; color: var(--secondary); i { color: var(--accent); font-size: 0.875rem; } &.stock-badge { background: rgba(16,185,129,0.1); border-color: rgba(16,185,129,0.3); color: #065f46; font-weight: 600; i { color: #059669; } } }

    .modal-form { display: flex; flex-direction: column; gap: 1rem; }
    .form-row { gap: 1rem; }
    .modal-actions-box { display: flex; align-items: center; justify-content: flex-end; gap: 0.75rem; margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-light); }
    .empty-state { .empty-icon { font-size: 3rem; color: var(--text-muted); margin-bottom: 0.5rem; } h4 { margin-bottom: 0.25rem; } }
    .spin-icon { animation: spin 1s linear infinite; }
    @keyframes spin { 100% { transform: rotate(360deg); } }

    @media (max-width: 992px) {
      .accordion-header { flex-direction: column; align-items: flex-start; gap: 1rem; }
      .accordion-header-right { width: 100%; justify-content: space-between; }
...
export class BranchInventoryComponent implements OnInit {
  ...
  public groupedInventory = computed<GroupedProductInventory[]>(() => {
    ...
    for (const item of list) {
      ...
      if (!group) {
        group = {
          productId: prodId, productName: prodName, productSku: prodSku,
          productCategory: prodCategory, productImage: prodImg,
          items: [], totalStock: 0, totalReserved: 0, totalSold: 0,
          hasLowStock: false, isOutOfStock: false, variantsCount: 0
        };
  ...
  loadInventory(): void {
    ...
    // El backend filtra automáticamente por la sucursal del encargado
  ...
  expandAll(): void {
    this.expandedProductIds.set(new Set(this.groupedInventory().map(g => g.productId)));
  }
  ...
  openRegisterMovementModal(item?: Inventario): void {
    ...
    this.movementForm.reset({ inventario_id: defaultId, tipo: 'ENTRADA', cantidad: 15, motivo: '' });
    this.isMovementModalOpen.set(true);
  }

  closeMovementModal(): void { this.isMovementModalOpen.set(false); }
  ...
  saveMovement(): void {
    ...
    this.inventoryApi.registerMovement(dto).subscribe({
      next: () => {
        this.toast.success('Movimiento de inventario registrado correctamente.');
  ...
  openQuickAdjustModal(item: Inventario): void {
    ...
    this.adjustForm.reset({ cantidad: item.cantidad, motivo: 'Ajuste de inventario por arqueo' });
    this.isAdjustModalOpen.set(true);
  }

  closeAdjustModal(): void { this.isAdjustModalOpen.set(false); this.selectedInventoryForAdjust.set(null); }
  ...
  openMovementsHistoryModal(): void {
    this.inventoryApi.getMovements(0, 100).subscribe({
      next: (movements) => { this.movementsHistory.set(movements); this.isHistoryModalOpen.set(true); },
      error: () => { this.movementsHistory.set([]); this.isHistoryModalOpen.set(true); }
    });
  }

  closeMovementsHistoryModal(): void { this.isHistoryModalOpen.set(false); }
}
